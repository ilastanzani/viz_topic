...
<script>
  ...
  // Fixed 11-color palette anchored at Mid (level 2)
  const MID_PALETTE = [
    '#8dd3c7','#ffffb3','#bebada','#fb8072','#80b1d3',
    '#fdb462','#b3de69','#fccde5','#d9d9d9','#bc80bd','#ccebc5'
  ];

  // Maps every aggCluster leaf id to its Mid-level color
  let colorById = new Map();

  function initHierarchyColors() {
    colorById = new Map();
    // Assign palette colors at Mid level (level 2), propagate to all leaf ids
    const mid = buildLevelClusters(2).sort((a, b) => a.id - b.id);
    mid.forEach((mc, idx) => {
      const color = MID_PALETTE[idx % MID_PALETTE.length];
      mc.ids.forEach(id => colorById.set(id, color));
    });
  }
  ...
  function drawBubble(el) {
    ...
    const parentByFineId = new Map();
    if (currentLevel === 1) {
      buildLevelClusters(2).forEach(parent => {
        parent.ids.forEach(id => parentByFineId.set(id, parent.id));
      });
    }
    ...
    _nodes = clusters.map(d => ({
      ...d,
      parentId: currentLevel === 1 ? (parentByFineId.get(d.id) ?? d.id) : null,
      r:  rOf(d),
      tx: xS(d.cx),
      ty: yS(d.cy),
      x:  xS(d.cx),
    ...
    // Run simulation with live tick callbacks — bubbles settle organically, then freeze
    simulation = d3.forceSimulation(_nodes)
  .force('x', d3.forceX(d => d.tx).strength(0.12))
  .force('y', d3.forceY(d => d.ty).strength(0.12))
  .force('groupX', currentLevel === 1
    ? d3.forceX(d => parentCenters.get(d.parentId)?.x ?? d.tx).strength(0.20)
    : null)
  .force('groupY', currentLevel === 1
    ? d3.forceY(d => parentCenters.get(d.parentId)?.y ?? d.ty).strength(0.20)
    : null)
  .force('collide', d3.forceCollide(d => {
    return d.r + (currentLevel === 1 ? 1 : 2);
  }).strength(0.85).iterations(4))
  .force('separate', currentLevel === 1 ? (() => {
  ...
  function drawSubLevel(el) {
    ...
    if (currentLevel === 1) {
      ...
      const fineClusters = buildLevelClusters(1);
      const subClusters  = fineClusters.filter(fc =>
        selNode.ids.some(id => fc.ids.includes(id))
      );
      if (!subClusters.length) return;

      const maxKwSub = d3.max(subClusters, d => d.kwcount) || 1;
      ...
      const subNodes = subClusters.map(sc => ({
        ...sc,
        r: rOfSub(sc),
      ...
      const subCircles = grp.selectAll('circle.sub-bubble').data(subNodes).join('circle')
        ...
        .attr('cx', d => d.x).attr('cy', d => d.y).attr('r', d => d.r)
        .attr('fill', d => colorById.get(d.id) ?? colorScale(d.id))
        .attr('opacity', 0.65)
        .attr('stroke', 'rgba(0,0,0,0.4)').attr('stroke-width', 0.8);
      ...
      _subSim = d3.forceSimulation(subNodes)
        ...
        .force('collide', d3.forceCollide(d => d.r + 1).strength(1).iterations(6))
        .force('contain', () => {
