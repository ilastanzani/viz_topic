...
<script>
  ...
  import { clusterKeywords, TREE, aggClusters, colorScale, grants, grantsByCluster } from '$lib/data.js';

<script lang="ts">
  import { onMount } from 'svelte';
  import { getClusterKeywords } from '$lib/data.js';

  let clusterKeywords = [];

  onMount(async () => {
    clusterKeywords = await getClusterKeywords();
    console.log('Loaded', clusterKeywords.length);
  });
</script>
  ...
  '#ccebc5','#ffed6f'
    ...
    coarse.forEach((cc, cIdx) => {
      ...
      _coarseColorById.set(cc.id, coarseBase.formatHex());
  ...
  // All siblings get exactly the coarse color (no variation)
  _fineColorById.set(fc.id, coarseBase.formatHex());
});
    });
  }
  ...
  function clusterColor(id) {
    ...
    if (currentLevel === 2) return _coarseColorById.get(id) ?? colorScale(id);
...
.attr('height', d => {
  ...
  return cellH - (topGap + bottomGap) / 2;
        ...
        .on('click',      (_, d) => {
          ...
          selectedGrants = grantsByCluster.get(selectedId) ?? [];
          hoveredId  = null;
...
.grants-scroll::-webkit-scrollbar       { width: 4px; }
.grants-scroll::-webkit-scrollbar-thumb { background: #c9d5e5; border-radius: 2px; }
  ...
  <div class="main">
   <div class="main-row">
    ...
    <div class="right-col">
      ...
      <!-- BOTTOM RIGHT: grants panel -->
      <div class="grants-panel">
        <div class="grants-panel-title">
          {#if selectedGrants.length > 0}
            {selectedGrants.length} grant{selectedGrants.length > 1 ? 's' : ''} — {listTitle}
          {:else}
            Grants
          {/if}
        </div>
        {#if selectedGrants.length > 0}
          <div class="grants-scroll">
            {#each selectedGrants as g}
              <div class="grant-card">
                <div class="grant-card-title">{g.title}</div>
                <div class="grant-card-kw">{g.keywords}</div>
              </div>
            {/each}
          </div>
        {:else}
          <div class="grants-empty">
            {selectedId !== null ? 'No grants for this cluster' : 'Click a cluster to see grants'}
          </div>
        {/if}
      </div>
