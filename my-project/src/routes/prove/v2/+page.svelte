<script>
  ...
  const colorScale = d3.scaleSequential(d3.interpolateTurbo).domain([0, 118]);

  let treemapEl = $state(null);
  let dendroEl = $state(null);

  // ── TREEMAP ──────────────────────────────────────────────────────────────────
  // Build hierarchy from TREE but use kwcount as leaf value
  function buildTreemapHierarchy() {
    function convert(node) {
      if ('id' in node) {
        return { id: node.id, name: node.kw0, value: node.kwcount, kw0: node.kw0 };
      }
      return {
        name: node.name,
        dist: node.dist,
  ...
  function drawTreemap(el) {
    ...
    const hier = d3.hierarchy(buildTreemapHierarchy())
      .sum(d => d.value ?? 0)
    ...
    // Only draw leaves (actual clusters)
    ...
    svg.selectAll('rect.node')
      ...
      .attr('opacity', d => {
        if (selectedId !== null) {
          return selectedId === d.data.id ? 1 : 0.25;
        }
        return hoveredId === d.data.id ? 1 : 0.85;
      })
      .attr('stroke', d => selectedId === d.data.id ? '#fff' : '#0f172a')
      .attr('stroke-width', d => selectedId === d.data.id ? 2 : 1)
      .style('cursor', 'pointer')
      .on('mouseenter', (_, d) => { if (selectedId === null) hoveredId = d.data.id; })
      .on('mouseleave', ()     => { if (selectedId === null) hoveredId = null; })
      .on('click',      (_, d) => { selectedId = selectedId === d.data.id ? null : d.data.id; hoveredId = null; });


    // ID label
    svg.selectAll('text.leaf-lbl')
      .data(leaves)
      .join('text').attr('class','leaf-lbl')
      .attr('x', d => d.x0 + 4)
      .attr('y', d => d.y0 + 12)
      .attr('font-size', d => (d.x1-d.x0) > 40 ? '9px' : '7px')
      .attr('fill', d => selectedId === d.data.id ? '#0f172a' : '#fff')
      .attr('font-weight', d => selectedId === d.data.id ? 'bold' : 'normal')
      .attr('pointer-events', 'none')
      .text(d => {
        const w = d.x1-d.x0, h = d.y1-d.y0;
        return (w < 18 || h < 12) ? '' : d.data.id;
      });

    // kw0 subtitle
    svg.selectAll('text.leaf-sub')
      .data(leaves)
      .join('text').attr('class','leaf-sub')
      .attr('x', d => d.x0 + 4)
      .attr('y', d => d.y0 + 23)
      .attr('font-size', '7px')
      .attr('fill', '#ffffff88')
  ...
  function drawDendrogram(el) {
    ...
    const m  = { top:14, right:12, bottom:24, left:38 };
    const iw = W - m.left - m.right;
    const ih = H - m.top  - m.bottom;
    const maxDist = TREE.dist;
    const root = d3.hierarchy(TREE, n => n.children);
    d3.cluster().size([iw, ih])(root);
    const yScale = d3.scaleLinear().domain([0, maxDist]).range([ih, 0]);
    root.each(node => { node.y = yScale(node.data.dist ?? 0); });
    ...
    g.selectAll('circle.leaf')
      ...
      .on('click',      (_, d) => { selectedId = String(selectedId) === String(d.data.id) ? null : d.data.id; })

    if (selectedId !== null) {
      const s = leaves.find(l => l.data.id === selectedId);
...
$effect(() => {
  if (!treemapEl || !dendroEl) return;
  const _sel = selectedId;  // explicit read so $effect tracks these
  const _hov = hoveredId;
  drawTreemap(treemapEl);
  drawDendrogram(dendroEl);
  ...
  let displayKws = $derived(activeId !== null ? (clusterKeywords[activeId] ?? []) : []);
  let listTitle  = $derived(
    selectedId !== null ? `Cluster ${selectedId} — ${displayKws.length} keyword`
    : hoveredId !== null ? `Cluster ${hoveredId} (hover)`
    : 'Clicca una cella per vedere le keyword'
  );
</script>
...
<div class="app">
  <div class="panel left">
    <div class="nav">
      <a href="/">Voronoi</a>
      <a href="/v2" class="active">Treemap</a>
      <a href="/v3">Bubble</a>
    </div>
    <div class="panel-title">Plot Cluster — Treemap</div>
    <div class="panel-body"><svg bind:this={treemapEl}></svg></div>
  </div>
